      ...
      <div class="minimal-card p-10 slide-up">
        <form @submit.prevent="submitForm" class="space-y-6">
          ...
          <div>
            ...
            <input v-model="form.email" type="email" :placeholder="$t('register.emailPlaceholder')" class="input-field"
              :class="{ 'input-error': errors.email }" @blur="validateEmail" />
          ...
          <div>
            ...
            <input v-model="form.username" type="text" :placeholder="$t('register.usernamePlaceholder')"
              class="input-field" :class="{ 'input-error': errors.username }" @blur="validateUsername" />
          ...
          <div v-if="config.authme?.enabled && config.authme?.require_password">
            ...
            <input v-model="form.password" type="password" :placeholder="$t('register.passwordPlaceholder')"
              class="input-field" :class="{ 'input-error': errors.password }" @blur="validatePassword" />
          ...
          <div>
            ...
            <div class="flex flex-col sm:flex-row gap-3">
              <input v-model="form.code" type="text" :placeholder="$t('register.codePlaceholder')"
                class="input-field flex-1" :class="{ 'input-error': errors.code }" @blur="validateCode" />
              <button type="button" @click="sendCode" :disabled="sending || !form.email || cooldownSeconds > 0"
          ...
          <!-- Submit button -->
          <button type="submit" :disabled="loading || !isFormValid" class="btn-primary w-full">
        ...
        <!-- Message display removed - now using notification system -->
      </div>
...
// Message state for backward compatibility
const message = ref('')
  ...
  try {
    ...
    if (data.success) {
      ...
      if (data.remaining_seconds && data.remaining_seconds > 0) {
        ...
        message.value = data.msg && !data.msg.includes(chineseSecond) && !data.msg.includes(secondsText) && !data.msg.includes(secondText) ? data.msg : ''
      } else {
...
const submitForm = async () => {
  ...
  try {
    ...
    if (data.success) {
      ...
      error(data.msg || t('register.failed'))
    }
  } catch (e) {
